...
<div class="container pb-30 pt-30">
		...
		@for($i = 0; $i < $classroom->weeks; $i++)
                <div class="course-single-item bg-white border-1px clearfix m-b-30 week-box" id="week-{{$i+1}}">
					    
					  <div class="course-details clearfix p-20 pt-15">

					                  <div class="course-top-part pull-left mr-40">
					                    <h4 class="mt-0 m-b-10"><b>Week {{$i+1}}</b><span class="topics-done-count">{{$classroom->chapters()->where('week', $i+1)->where('status', 2)->count()}}/{{$classroom->chapters()->where('week', $i+1)->count()}}</span></h4>
